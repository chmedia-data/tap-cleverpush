# AGENTS.md - AI Agent Development Guide for tap-cleverpush
...
This tap follows the Singer specification and uses the Meltano Singer SDK to extract data from CleverPush.

### Key Components

1. **Tap Class** (`tap_cleverpush/tap.py`): Main entry point, defines streams and configuration
1. **Client** (`tap_cleverpush/client.py`): Handles API communication and authentication
1. **Streams** (`tap_cleverpush/streams.py`): Define data streams and their schemas
...
1. Define stream class in `tap_cleverpush/streams.py`
...
```python
class MyNewStream(CleverPushStream):
    name = "my_new_stream"
...
1. **SimpleHeaderPaginator**: For APIs using Link headers (RFC 5988)
   ...
   class MyStream(CleverPushStream):
...
```bash
tap-cleverpush --config config.json --discover
tap-cleverpush --config config.json --catalog catalog.json
```
...
1. Update `config_jsonschema` in `tap_cleverpush/tap.py`
...
```bash
# .env.example
TAP_CLEVERPUSH_API_URL=https://api.example.com
TAP_CLEVERPUSH_API_KEY=your_api_key_here
TAP_CLEVERPUSH_BATCH_SIZE=100  # New setting
```
